...
std::vector<T> ferrari(fourth_degree_polynomial<T> P)
{
	...
	if (q == (0, 0))
	{
		...
		std::complex<float> D1 = p * p - 4 * r;
		roots[1] = -sqrt((-p + sqrt(D1)) * 0.5) - coefs[3] * 0.25;
		roots[2] = -sqrt((-p - sqrt(D1)) * 0.5) - coefs[3] * 0.25;
		roots[3] = sqrt((-p + sqrt(D1)) * 0.5) - coefs[3] * 0.25;
		roots[4] = sqrt((-p - sqrt(D1)) * 0.5) - coefs[3] * 0.25;
